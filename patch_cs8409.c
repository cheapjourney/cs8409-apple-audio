...
void cs8409_cs42l42_fixups(struct hda_codec *codec, const struct hda_fixup *fix, int action)
{
	...
	case HDA_FIXUP_ACT_PRE_PROBE:
		...
		spec->callbacks.unsol_event = cs8409_cs42l42_jack_unsol_event;
...
void dolphin_fixups(struct hda_codec *codec, const struct hda_fixup *fix, int action)
{
	...
	case HDA_FIXUP_ACT_PRE_PROBE:
		...
		spec->callbacks.unsol_event = dolphin_jack_unsol_event;
...
// dispatcher helpers – use callbacks struct (same offset in both spec types)
struct cs8409_spec;
static inline void *cs8409_spec_callbacks(struct hda_codec *codec)
{
	return codec->spec ? (void *)&((struct cs8409_spec *)codec->spec)->callbacks : NULL;
}

#define CS8409_CB(codec, field) \
	({ void *_cb = cs8409_spec_callbacks(codec); \
	   _cb ? ((typeof(&((struct cs8409_spec *)0)->callbacks))_cb)->field : NULL; })

static void cs8409_unsol_event(struct hda_codec *codec, unsigned int res)
{
	void (*cb)(struct hda_codec *, unsigned int) = CS8409_CB(codec, unsol_event);
	if (cb)
		cb(codec, res);
	else
...
static int cs8409_build_controls_dispatch(struct hda_codec *codec)
{
	int (*cb)(struct hda_codec *) = CS8409_CB(codec, build_controls);
	return cb ? cb(codec) : cs8409_build_controls(codec);
}

static int cs8409_build_pcms_dispatch(struct hda_codec *codec)
{
	int (*cb)(struct hda_codec *) = CS8409_CB(codec, build_pcms);
	return cb ? cb(codec) : snd_hda_gen_build_pcms(codec);
}

static int cs8409_init_dispatch(struct hda_codec *codec)
{
	int (*cb)(struct hda_codec *) = CS8409_CB(codec, init);
	return cb ? cb(codec) : cs8409_init(codec);
}

static void cs8409_remove_dispatch(struct hda_codec *codec)
{
	void (*cb)(struct hda_codec *) = CS8409_CB(codec, remove);
	if (cb)
		cb(codec);
	else
		cs8409_free(codec);
...
static int cs8409_suspend_dispatch(struct hda_codec *codec)
{
	int (*cb)(struct hda_codec *) = CS8409_CB(codec, suspend);
	return cb ? cb(codec) : cs8409_cs42l42_suspend(codec);
}

static int cs8409_resume_dispatch(struct hda_codec *codec)
{
	int (*cb)(struct hda_codec *) = CS8409_CB(codec, resume);
	return cb ? cb(codec) : 0;
}

static const struct hda_codec_ops cs8409_codec_ops = {
	.probe = patch_cs8409,
	.remove = cs8409_remove_dispatch,
	.build_controls = cs8409_build_controls_dispatch,
	.build_pcms = cs8409_build_pcms_dispatch,
	.init = cs8409_init_dispatch,
	.unsol_event = cs8409_unsol_event,
	.suspend = cs8409_suspend_dispatch,
	.resume = cs8409_resume_dispatch,
};
